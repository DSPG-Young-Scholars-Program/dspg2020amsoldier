...
# Project Goals
_dspg team, sdad team, stakeholders_

# Overview

The American Soldier project is a digital history project funded by the National Endowment of Humanities.The goal of the overall project is to take a range of surveys and commentaries by WW2 soldiers to analyze a wide range of topics from race and gender relations, career plans, thoughts on South East Asia, and much more. 

Led by Professor Ed Gitre. Also, Professor Mark Embree of Virginia Tech uses machine learning to unpack and understand the text data generated by the soldiers. 

This summer, our primary goal has been to understand and analyze

* race relations 
* gender relations
* and the intersection between race and space

using a collection surveys that discuss these issues.

_reformat this section, look at the midterm presentation_
_insert picture of powerpoint slide to inlcude about the team_

# Data & Methods 
The surveys have been transcribed through Zooniverse, which is a crowdsourced trascription site.

* Survey 32
    + administered to both black and white soldiers
    + goal: to study the attitudes of and towards black soliders, specifically within the context of desegregating the military
    + a question of interest: "Do you think white and black soldiers should be in separate outfits?"
        - multiple choice
        - 7,442 responses from black soldiers
        - 4,678 responses from white soliders, who could also provide free form commentary
    + longer comment on the overall survey
        - 3,464 responses from black soldiers
        - 2,324 responses from white soldiers
        - average response length was shorter than 100 words
_include photo to show what the survey looks like before it is transcribed_
* Survey 144
    + post-war career plans of black soliders
    + don't have transcriptions for free form questions
* Survey 190
    + attitudes towards women
    + don't have transcriptions for free form questions
    + question examples
        - "Is the army any place for a girl to be?" 
        
_methods could be the ones from the slids, very basic_
## Summary Statistics
_add info / stats about soceioeconomic info_

## Text Cleaning
_how many responses to we have? what is the size of the data_
_what did we do to clean the data? how many words did we start with? how many words did we end up with after we finished cleaning?_

# Methods

To analyze the text data, we focused on three techniques: 

* sentiment analysis
    + [NRC dictionary](https://saifmohammad.com/WebPages/NRC-Emotion-Lexicon.htm) sentiment lexicon 
        - anger, fear, anticipation, trust, surprise, sadness, joy, disgust, negative, positive 
    + comparing sentiment distributions across different groups
        - black and white soldiers
        - pro-segregation and anti-segegration white soldiers
        - unique words 
        - responses that dicuss gender
* text networks
    + bigrams and co-occurences used across different groups
        - black and white soldiers
        - pro-segregation and anti-segregation white soldiers
        - responses that discuss gender
        - repsonses that discuss race and space
* topic modeling
    + LDA, biterm topic modeling, and GEPHI 

These three tools help us glean insights into the feelings of soldiers, the frequencies of terms they used, and which overall topics they discussed. The text analysis is completed on Survey 32 text data and separates soldiers into four main groups of text

* black soldiers' long responses
* white soldiers' long responses
* pro-segregation white soldier's short responses 
* anti-segregation white soldier's short responses. 

## Race Relations
_sentiment analysis_
_start with radar chart_
_then unique word clouds_
...
_community detection algorithms to see certain patterns, take graphs from midterm presentation_

### Topic Modeling
_bullets describing what we are doing_
...
_take out gender sentiment radar_
...
## Spatial Arrangement
_display spatial dictionary like gender one_
_add descriptoin of methods_
...
## Conclusion
_data from a unique and historic collection that was produced by the U.S. Army during the war_
_unique, historic data collection_
_novel dataset_
_summarize key findings_
_analyze soldiers’ handwritten responses by using natural language processing methods and social network analysis to dive into soldiers’ attitudes about racial and gender relations_
_one or two bullets for each topic: race, gender, space_
