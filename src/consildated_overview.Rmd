...
enumerate_attribute = function(df=w_ans,df2=b_ans, att, att2){
  df %>% 
  count(att, sort =T) %>% 
  na.omit() %>% 
  left_join(df2 %>% count(att, sort =T) %>% na.omit(),by = att2) %>%
  melt(id.vars= att2) 
}
```
...
ggplot( aes(x=edu, fill = age)) +geom_bar(position= "fill") +
  ...
  labs(title="Age Distribution over Education Levels of Black Soldiers", x="State", y = "Proportion")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_brewer(palette="YlGnBu")
